...
class TulajdonosController extends AbstractController
{
     ...
     */
    public function getAllTulajdonos(ManagerRegistry $registry, SerializerInterface $serializer): Response
    {
        $tulajdonosService = new TulajdonosService();
     ...
     */
    public function getTulajdonos(ManagerRegistry $registry, SerializerInterface $serializer, $id): Response
    {
        $tulajdonosService = new TulajdonosService();
     ...
     */
    public function deleteTulajdonos(ManagerRegistry $registry, $id): Response
    {
        $tulajdonosService = new TulajdonosService();
     ...
     */
    public function getAllTulajdonosEszkoz(ManagerRegistry $registry, SerializerInterface $serializer, $id): Response
    {
        $tulajdonosService = new TulajdonosService();
    ...
    /**
     * @Route("/api/tulajdonos/{id}", methods={"PUT"})
     */
    public function updateTulajdonos(Request $request, ManagerRegistry $registry, $id): Response
    {
        if($request->headers->get('Content-Type') != 'application/json') {
            return new Response('A HTTP body üres vagy nem megfelelő a Content-Type!', 406);
        }
        ...
        $data = json_decode($request->getContent(),true);
        $tulajdonosRequestDto = new TulajdonosRequestDto($data['nev'], $data['szemelyi'], date_create($data['szuldatum']));
        $state = $tulajdonosService->updateTulajdonos($registry, $id, $tulajdonosRequestDto, $data["id"]);

        if($state === 1) {
            return new Response('A tulajdonos frissítése sikeres!', 201);
        } else if($state === 2) {
            return new Response('Ez a tulajdonos nem létezik!', 404);
        } else if($state === 0) {
            return new Response('Ezt a tulajdonost nem szerkesztheted!', 403);
        }
